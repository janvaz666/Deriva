...
namespace api.Controllers
{
    ...
    public class ConvenioController : ControllerBase
    {
        ...
        private readonly IConvenioRepository convenioRepository;
        ...
        public async Task<IActionResult> GetConvenio()
        {
            var response = new ResponseConvenio();

            try
            {
                response.Code = 200;
                response.convenio = await convenioRepository.GetConvenio() as List<ConvenioEntity>;
                response.Message = "OK";
            }
            catch (Exception ex)
            {
